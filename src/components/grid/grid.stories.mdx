...
## Examples
The following examples are best viewed in the Canvas tab using full-screen mode with device or viewport emulation.

### Basic
A simple structure might define settings for a `maxWidth: 1500px`. With each `GridItem` defined to cover six columns and one row they appear side-by-side up to a `maxWidth: 1500px`. Beyond 1500px they obey the default settings of occupying 12 columns, and each `GridItem` on a new row.
<Preview>
...
### Full
This example demonstrates more refined control. Here the order and position of each `GridItem`, responds according to an array of `responsiveSettings` allowing reordering and respositioning as required.
<Preview>
  <Story name="full" parameters={{ info: { disable: true }}} >
    <GridContainer>
      <GridItem responsiveSettings={[
        {
          maxWidth: '1500px',
