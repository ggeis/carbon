...
import {
  StyledLeftButtons, StyledRightButtons, StyledFormFooter, StyledForm
} from './form.style';
...
describe('Form', () => {
  ...
  it('cleans up event listeners after unmounting', () => {
    ...
    expect(removeEventListenerSpy.mock.calls.filter(
      call => call[0] === 'scroll' || call[0] === 'resize'
    )).toHaveLength(2);
    expect(ElementResizeSpy.mock.calls.filter(call => call[0] === formNode)).toHaveLength(1);
  });
  ...
  describe('when stickyFooter prop is true', () => {
    ...
    it('renders form footer with proper styles if form is bigger than window', () => {
      const formNode = wrapper.find(StyledForm).getDOMNode();
      jest.spyOn(formNode, 'offsetHeight', 'get').mockImplementation(() => 1000);
      jest.spyOn(formNode, 'offsetTop', 'get').mockImplementation(() => 100);
    ...
    it('renders form footer with proper styles if form is smaller than window', () => {
      const formNode = wrapper.find(StyledForm).getDOMNode();
      jest.spyOn(formNode, 'offsetHeight', 'get').mockImplementation(() => 300);
      jest.spyOn(formNode, 'offsetTop', 'get').mockImplementation(() => 100);
    ...
    it('does not change stickyFooter state if it does not need to change', () => {
      ...
      jest.spyOn(formNode, 'offsetHeight', 'get').mockImplementation(() => 900);
      jest.spyOn(formNode, 'offsetTop', 'get').mockImplementation(() => 100);
      jest.spyOn(footerNode, 'offsetHeight', 'get').mockImplementation(() => 100);
  ...
  describe('form buttons', () => {
    ...
    it('renders buttons passed as the leftSideButtons prop', () => {
      expect(wrapper.find(StyledFormFooter).childAt(0).is(StyledLeftButtons)).toBe(true);
      expect(wrapper.find(StyledLeftButtons).find(Button).at(0).props().id).toBe('myLeftButton1');
      expect(wrapper.find(StyledLeftButtons).find(Button).at(1).props().id).toBe('myLeftButton2');
    });

    it('renders button passed as the saveButton prop wrapped by the Form Summary by default', () => {
      expect(wrapper.find(StyledFormFooter).childAt(1).is(FormSummary)).toBe(true);
      expect(wrapper.find(StyledFormFooter).find(FormSummary).find(Button).props().id).toBe('mySaveButton');
    });

    it('renders button passed as the saveButton prop not wrapped by FormSummary when showSummary set as false', () => {
      wrapper.setProps({ showSummary: false });
      expect(wrapper.find(FormSummary).exists()).toBe(false);
      expect(wrapper.find(StyledFormFooter).childAt(1).find(Button).props().id).toBe('mySaveButton');
    });

    it('renders buttons passed as the rightSideButtons prop', () => {
      expect(wrapper.find(StyledFormFooter).childAt(2).is(StyledRightButtons)).toBe(true);
      expect(wrapper.find(StyledRightButtons).find(Button).at(0).props().id).toBe('myRightButton1');
      expect(wrapper.find(StyledRightButtons).find(Button).at(1).props().id).toBe('myRightButton2');
    });
  });
  ...
  describe('styles', () => {
    ...
    it('form summary has proper background when showSummary and errorCount or warningCount are passed', () => {
      wrapper.setProps({ showSummary: true, warningCount: 1 });
      assertStyleMatch({
        backgroundColor: baseTheme.form.invalid
      }, wrapper.find(StyledFormSummary));
      wrapper.setProps({ showSummary: true, errorCount: 1 });
      assertStyleMatch({
        backgroundColor: baseTheme.form.invalid
      }, wrapper.find(StyledFormSummary));
    });
  ...
  describe('FormSummary', () => {
    describe('when showSummary prop is passed as false', () => {
      it('renders save button without FormSummary', () => {
        const saveButton = <Button id='mySaveButton'>Save</Button>;

        wrapper.setProps({ saveButton, showSummary: false });
        expect(wrapper.find(FormSummary).exists()).toBe(false);
        expect(wrapper.find(Button).props().id).toBe('mySaveButton');
      });
    });

    describe('when showSummary prop is passed as true', () => {
      it('renders save button without FormSummary', () => {
        const saveButton = <Button id='mySaveButton'>Save</Button>;

        wrapper.setProps({ saveButton, showSummary: true });
        expect(wrapper.find(FormSummary).find(Button).props().id).toBe('mySaveButton');
      });
      ...
      describe('when errorCount prop is set on Form', () => {
        it('renders error summary with properly colored icon and text', () => {
          ...
          expect(errorSummary.find(StyledSummaryText).text()).toBe('There is 1 error');
      ...
      describe('when warningCount prop is set on Form', () => {
        it('renders warning summary with properly colored icon and text', () => {
          ...
          expect(warningSummary.find(StyledSummaryText).text()).toBe('There is 1 warning');
      ...
      describe('when either errorCount or warningCount or both are set on Form', () => {
        it.each([
          ...
          (errorCount, warningCount, errMessage, warnMessage) => {
            ...
            if (errorCount) {
              expect(wrapper.find(StyledSummaryText).at(0).text()).toBe(errMessage);
            }
            if (warningCount) {
              expect(wrapper.find(StyledSummaryText).at(warningPosition).text()).toBe(warnMessage);
            }
          }
        );
      });
  ...
  describe('tags', () => {
    ...
    it('include correct data-element tags on elements', () => {
      expect(tagWrapper.find(StyledFormFooter).prop('data-element')).toBe('form-footer');
      expect(tagWrapper.find(StyledFormSummary).prop('data-element')).toBe('form-summary');
      expect(tagWrapper.find(StyledInternalSummary).at(0).prop('data-element')).toBe('errors');
