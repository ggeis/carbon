...
const warningAppend = ({ errors, warnings }, type) => errors > 0 && warnings > 0 && type === 'warning';

const translationKey = (props, type) => (warningAppend(props, type) ? 'errors_and_warnings' : pluralize(type));
...
const Summary = ({ type, ...props }) => {
  if (props[pluralize(type)] > 0) {
    return (
      <StyledInternalSummary type={ type } data-element={ pluralize(type) }>
        <Icon type={ type } />
        <StyledSummaryText>
